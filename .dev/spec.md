...
## 6.23 ビューポート第8弾 — スケール第2弾: 1〜2億三角形(Phase 23)

(2026-08-01 の設計インタビューで決定)

### 6.23.1 テーマ選定と性能目標

- **スケール第2弾**を採用。Phase 22 の実測(5,000万で WorkingSet 5.8GB・30fps)の単純外挿では 2億は WS 約 23GB・約 7fps で破綻するため、メモリ減量と描画三角形数の削減という 2 つの独立したボトルネック対策が必要
- **目標: 2億三角形・1億節点**(RAM 32GB+/VRAM 12GB+ 想定)。ロード数十秒以内・**操作中(LOD)30fps**・静止時フル描画は秒単位を許容。Release ベンチは手元環境の上限まで計測して記録
- Nanite 型のクラスタ階層 LOD+アウトオブコアによる「数億〜無制限」は研究開発規模のため引き続き将来フェーズ

### 6.23.2 メモリ減量(2 点)

1. **法線の octahedral 16bit×2 圧縮**(頂点 28B→20B、1億節点で GPU 800MB 減)。エンコード/デコードは純粋関数化して往復誤差をテスト
2. **構築中間のストリーミング化**: フルサイズ中間配列(ローカル座標/法線)をやめてチャンク単位生成にし、CPU ピークを「頂点総数比例」から「チャンクサイズ比例」へ
- 変位 half3 化・位置のチャンク相対 snorm16 量子化は効果対リスク比から見送り(バックログ)
- アプリが持つ入力 double 配列(2億で位置 2.4GB+インデックス 2.4GB)はアプリ責務のため対象外。GPU に渡るのは従来どおり再センタリング後の float

### 6.23.3 操作中 LOD(グリッド頂点クラスタリング)

- **単一 LOD(目標約 1/20)** をロード時に並列構築。一様グリッドのセル内頂点を 1 点に束ね、退化三角形を除去。グリッド解像度はモデル AABB から内部ヒューリスティックで逆算
- **カメラ操作中(回転/パン/ズーム/視点アニメ)と振動アニメ中は LOD、操作終了で 1 フレームかけてフル描画**(CAE ビューアの定番方式)
- LOD 頂点は**スカラー平均+代表節点の変位**を持ち、コンター/変形表示/モード振動/過渡再生も LOD で成立する。エッジ重畳/グリフ/非変形重畳は LOD 描画中は非表示
- **ピック/プローブ/選択ハイライトは常にフルメッシュ**(クリック時の単発コストのため精度を落とさない)
- 閾値(三角形数)未満のメッシュは LOD 自体を構築しない

### 6.23.4 チャンクフラスタムカリング

- 既存の連続分割チャンクに **AABB を持たせてフラスタム判定**(描画パス・ID パス両方)。三角形の並び順を変えないため、ピック整合・メモリへの影響ゼロ
- メッシャー由来の CAE メッシュは番号順に空間的局所性を持つのが普通で、そのままでもズームイン時に有効
- Morton 順並べ替えによるカリング効率最大化は、ピック ID 復元に置換表(2億で約 800MB)が必要になるため見送り(バックログ)

### 6.23.5 API

- **`WcuViewport.InteractiveLodThreshold` DP**(三角形数、既定 500万、int.MaxValue で無効)のみ新設。間引き率は内部ヒューリスティック、カリングは常時有効(DP なし)
- **`ViewportStatistics` 拡張**: LOD 三角形数 / LOD 描画中か / 直近の描画チャンク数(カリング後)

### 6.23.6 デモ — 「3D Benchmark」ページ拡張

- サイズ選択に **1億/2億を追加**(メモリ消費の注記付き)+ LOD/カリング統計表示 + **LOD 無効トグル**(InteractiveLodThreshold = int.MaxValue の参照実装)+ 振動アニメで変形 LOD 確認
- 回転アニメ FPS 計測がそのまま「LOD 時の操作 fps」の計測になる

### 6.23.7 検証

- **xUnit**: グリッドクラスタリング(セル割当/退化除去/スカラー平均/代表変位)、octahedral 法線圧縮の往復誤差、AABB フラスタム判定、LOD 閾値判定(全て純粋関数)
- **UIA**: 回転アニメ中に統計文字列が LOD 描画中になることをアサート / 静止後フル描画復帰(LOD 無効時との決定性ピクセル一致) / カリング有効時のピック整合 / 両テーマスクショ
- **Release ベンチ**: 手元環境の上限まで(1億/2億)構築時間・LOD 中 FPS・静止時フル描画時間・メモリを計測して spec に記録

### 6.23.8 実装メモ(2026-08-01 完了)

- **octahedral 法線圧縮**(`ViewportGeometry.EncodeOctahedralNormal` / `DecodeOctahedralNormal` / `CompressNormals`): 16bit×2 を uint 1 個にパック、頂点 28B→20B。HLSL 側は `DecodeOctNormal`(入力レイアウトは `R32_UInt`)。往復誤差は単体テストで 1° 未満を確認
- **構築中間のストリーミング化**: 法線は `ComputeVertexNormalsFromDouble` で double 入力から直接計算(フルサイズのローカル float 座標配列を排除)→即 octahedral 圧縮して float 法線を解放。頂点インターリーブはチャンク単位に double からギャザー+再センタリング。変位バッファは**遅延生成**とし、変位なしメッシュは共有ゼロバッファ(stride 0 バインド)で GPU 確保ゼロ
- **ViewportLod**(internal 静的クラス): 一様グリッド頂点クラスタリングの純粋関数群。セルサイズは AABB 最大辺基準の立方(薄い軸の過分割を防ぐ)、解像度は `√(頂点数/削減率)` ヒューリスティック(既定 1/20)。ブロック並列+順序確定マージで**コア数によらず決定的**。LOD 頂点はスカラー平均+代表節点(変位追従用)を保持、退化/重複三角形を除去
- **LOD 描画統合**: `Camera.Changed`・振動アニメ tick で `NotifyInteractiveChange` → LOD 描画、停止 300ms 後の DispatcherTimer でフル復帰。LOD 中はエッジ/グリフ/非変形重畳を抑制。ピック/プローブ/矩形選択/ハイライトは常時フルメッシュ。変位更新は LOD チャンク(代表節点ギャザー)にも反映
- **ViewportCulling**(internal 静的クラス): Gribb-Hartmann 法でフラスタム 6 平面抽出+AABB 判定の純粋関数。チャンクごとに AABB を持ち、描画/ID 両パスでカリング。変形分は `MaxDisplacementMagnitude × |DeformationScale|` で AABB を保守的に拡張
- **API**: `WcuViewport.InteractiveLodThreshold` DP(既定 500万、int.MaxValue で無効)。`ViewportStatistics` に `LodTriangleCount` / `IsLodActive` / `LastDrawnChunkCount` を追加
- **ギャラリー**: Benchmark ページに 1億/2億+「操作中 LOD」トグル+LOD/カリング統計を追加。統計タイマーは Background だと回転アニメ(毎フレーム `CompositionTarget.Rendering`)中に飢餓して更新が止まるため **Normal 優先度に変更**(UIA で発覚)
- **Release ベンチ結果(2026-08-01、ハードウェア GPU、MSAA 4x、LOD 閾値 500万)**:

| 三角形数 | 節点数  | チャンク | 構築時間  | 静止フル描画 | LOD 三角形 | 回転(LOD)FPS | WorkingSet |
| -------- | ------- | -------- | --------- | ------------ | ---------- | ------------ | ---------- |
| 100万    | 50万    | 1        | 164 ms    | 2.6 ms       | (なし)     | 1,056        | 344 MB     |
| 1,000万  | 500万   | 2        | 592 ms    | ~22 ms       | 85.8万     | 1,021        | 1.2 GB     |
| 2,500万  | 1,250万 | 4        | 1,770 ms  | ~10 ms       | 214万      | 880          | 3.1 GB     |
| 5,000万  | 2,500万 | 7        | 2,350 ms  | ~14 ms       | 429万      | 574          | 6.5 GB     |
| 1億      | 5,001万 | 13       | 4,880 ms  | ~28 ms       | 857万      | 360          | 11.6 GB    |
| 2億      | 1億     | 26       | 10,584 ms | ~45 ms       | 1,714万    | 198          | 25.0 GB    |

- **目標(2億三角形・1億節点、ロード数十秒以内・操作中 30fps)を大幅達成**: 構築 10.6 秒・回転 198 FPS。Phase 22 比で 5,000万の WorkingSet 5.8→6.5GB は LOD 分を含む(頂点 20B 化で GPU 側は減、アプリ入力 double は不変)。表中の静止フル描画は回転直後のフレーム実測で、フラスタムカリング後の描画チャンク数に依存(2億 Top ビュー全景で 26 中 13 チャンク描画)
...
## 6.26 CaeStudio — 完全 MVVM+レイヤードアーキテクチャのサンプル CAE アプリ(Phase 26)

(2026-08-02 の設計インタビューで決定)

### 6.26.1 テーマ選定

- 残バックログにブロッカー級はなし(小粒改善=実需待ち / クラスタ階層 LOD=研究開発級 / 出荷準備=API 実戦検証後が正順)。これまでのデモは全てコードビハインド直結で、**「MVVM で使えるライブラリか」は一度も検証されていない**穴を塞ぐのが本フェーズ
- **簡易 FEM ソルバ内蔵のミニ CAE アプリ「CaeStudio」**を新設: プリ(メッシュ生成/境界条件)→ソルブ(非同期実行+進捗+キャンセル)→ポスト(コンター/変形/プローブ)の全ワークフローを持ち、R3 のリアクティブ層(残差ストリーム→収束チャート等)に実体を与える
- サンプルは MVVM 対応不足の**検出器**を兼ねる: アプリ側アダプタで書き切りつつ、「MVVM 的に書けない」と判明した箇所は**ライブラリ側の API 改善を積極的に実施**(ライブラリが主役のリポジトリ)
...
### 6.26.3 技術スタック(MVVM+リアクティブ)

- **R3 中心**: `BindableReactiveProperty<T>` / `ReactiveCommand` で VM を構成。残差→`Throttle`/`ObserveOnDispatcher`→チャート、選択状態→`CombineLatest`→コマンド可否、入力ストリーム合成→`IsDirty` 等のリアクティブ合成が主役
- コレクションは ObservableCollections(`ObservableList`+WPF ビュー)
- DI は Microsoft.Extensions.DependencyInjection + **Generic Host**(設定・ログも Host に乗せる)
- ライブラリ橋渡し: `Microsoft.Xaml.Behaviors.Wpf` ベースのアダプタ層(例: ViewportSelectionBehavior、イベント→R3 Observable 変換)。吸収不能な箇所はライブラリへ DP/コマンド/Observable 公開を還元
- パッケージは NuGet 最新安定版。アプリ UI 言語は日本語

### 6.26.4 ソルバ(Domain)

- **2D 平面応力 FEM**(CST 三角形要素): 静解析=CG 反復法(Jacobi 前処理、残差をストリーム通知)、固有値解析=集中質量行列+逆反復法(CG 再利用+Gram-Schmidt 直交化、下位 3〜5 モード)。`Parallel.For` 程度の軽い並列化
- 過渡応答(Newmark 法)は後続フェーズの拡張候補
- **形状テンプレート 2 種**(どちらも厳密解で検証可能):
  1. **円孔付き平板**(板寸法/孔径/引張応力/メッシュ密度をパラメトリック) — 静解析。Kirsch 厳密解と xUnit・パスプロットで比較
  2. **片持ち板**(長さ/幅/板厚/分割数) — 固有値解析。Euler-Bernoulli 理論値と比較、モードアニメ・FRF 合成のデータ源
- **境界条件はテンプレート定義の辺グループ**(幾何条件で定義され再メッシュ自動追従)に値のみ編集(荷重は Vector3Box)。ビューポート選択→BC 割り当ては幾何領域選択の設計が必要なため将来拡張

### 6.26.5 UI 構成(Docking IDE 型シェル+全コントロール網羅)

- Phase 25 シェルと同型構成を **MVVM で書き直す対比**: ModelTree(プロジェクトツリー)/PropertyGrid/収束チャート/ログ/ビューポートドキュメント/StatusBar+実行系ツールバー(ReactiveCommand CanExecute の見せ場)
- **既存全コントロールを最低 1 回登場させる**:
  - プリ: Wizard+StepIndicator(新規解析ウィザード) / NumericBox・NumericText・IUnitProvider(単位付き入力) / Vector3Box(荷重) / MatrixBox(D 行列) / PropertyGrid 一式 / ModelTree+SearchBox / ColorPicker・ColorEditor
  - ソルブ: SplitButton(実行) / ProgressDisplay・ProgressRing・BusyOverlay / ConvergenceMonitor(残差ライブ) / LogConsole / InfoBar(再解析必要の警告) / ToastHost(完了通知)
  - ポスト: WcuViewport フル機能(コンター/変形/振動アニメ/断面/グリフ/プローブ/選択/ViewCube) / ColorMapLegend+ColorScaleEditor+RangeSlider / WcuPlot(パスプロット vs Kirsch) / FrequencyResponsePlot(モード重ね合わせ FRF) / HistogramChart(応力度数分布) / HistoryChart(細分化スタディ) / PlaybackBar(モード位相スイープ) / CheckComboBox / DropDownButton(視点) / DataGridAssist(固有振動数・反力一覧)
  - シェル: WcuWindow / WcuDialogWindow+WcuMessageBox(設定/確認) / KeyGestureBox+PathBox(設定ダイアログ) / ToggleSwitch / Docking+DockLayout / ThemeManager

### 6.26.6 永続化(Infrastructure)

- **プロジェクトファイル**(`.wcuproj`、System.Text.Json): 入力のみ(形状パラメータ/材料/BC 値/メッシュ設定/解析タイプ)。結果は保存せず開いたら再解析(入力から決定的に再現可能)
- **ユーザ設定 JSON**: テーマ / 最近使ったファイル / ショートカット / 既定保存先
- **ドックレイアウト**: `DockLayout.SaveToString` を設定に同居保存
- ファイルメニュー一式(開く/保存/名前を付けて保存/最近使ったファイル)+未保存確認(IsDirty は R3 合成)

### 6.26.7 検証

- **xUnit(CaeStudio.Tests)**: ① Domain=応力集中係数の Kirsch 解収束・固有振動数の Euler-Bernoulli 一致・CG 収束性 ② Application=解析ユースケースの状態遷移/キャンセル/R3 ストリーム ③ ViewModel=ReactiveCommand CanExecute 遷移・ダーティ管理(WPF なしで VM が検証できること自体が MVVM 化の成否判定)
- **UIA 新設 `verify-caestudio.ps1`**: 起動→ウィザード新規→解析実行→完了トースト→コンターのピクセル検証→プローブ→保存/再読込ラウンドトリップ→両テーマスクショ
- **既存 UIA 全回帰**(ライブラリ側 API 変更を伴うため必須)

### 6.26.8 進め方(単一フェーズ・3 マイルストーン)

1. **M1 骨格**: 5 プロジェクト+DI/Host+ドッキングシェル+メッシュ生成+静解析ソルバ+厳密解 xUnit。ウィザード→メッシュ→解析→コンターの最小 E2E
2. **M2 機能拡充**: 固有値解析+ポスト処理一式(チャート 4 種/プローブ/凡例/RangeSlider/固有値テーブル/PlaybackBar 位相スイープ)+収束モニタ R3 ライブ更新
3. **M3 仕上げ**: 永続化+設定ダイアログ+ライブラリ MVVM 還元の確定+UIA+全回帰+spec 記録

各マイルストーン到達点でビルド+テスト green を維持する。

### 6.26.9 実装記録

- **M1 完了 (2026-08-01)**: Domain(Mesh2D/MeshGenerator/CSR/CG/FemModel/StaticAnalysis/ExactSolutions)+Application(AnalysisRunner/ProjectStore)+App シェル(DI/Host、ドッキング、ウィザード、PropertyGrid 連動、収束モニタ ライブ)。Kirsch 応力集中・梁たわみの厳密解 xUnit green。`verify-caestudio.ps1` で E2E 検証
